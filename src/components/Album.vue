<template>
  <div class="pa-4">
    <profile />
      ...
      <template v-for="el of rovers">
        <v-card
          outlined
          width="30%"
          :class="el.value===selectedRover.value?'primary black--text':''"
          :key="el.value"
          class="px-4 pointer"
          @click="selRover(el)"
        >
     ...
     <v-card outlined  height="100%" class="d-flex pa-2 align-center">
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            ...
            <v-text-field v-model="selDate" v-bind="attrs"
            type="number"
                  v-on="on" hide-details  dense outlined color="primary" />
          </template>
          <div style="width:400px" class="small-text text-left">Mars Solar Days and 24-hr Clock Convention A Mars solar day has a mean period of 24 hours 39 minutes 35.244 seconds, and is customarily referred to as a "sol" in order to distinguish this from the roughly 3% shorter solar day on Earth.</div>
        </v-tooltip>
      
        <v-btn color="primary" class="black--text font-weight-bold mx-1" @click="setDate">Search</v-btn>
      </v-card>
          </v-col>
      </v-row>
   
    <div class="d-flex">
      <h3 style="width: 100%;" class="uppercase ma-3">Select A Rover Camera</h3>
      <template v-for="el of roverTypes">
        <v-card
          outlined
          width="100%"
          :class="el.value===selecetedRoverType.value?'primary black--text':''"
          :key="el.value"
          class="px-4 pointer"
    ...
    <div class="d-flex justify-end my-4">
      ...
      <v-btn outlined width="100px" color="primary" :disabled="!data.length  || page<2" @click="clickPrev">
        Previous
      </v-btn>
       <v-btn outlined width="100px" color="primary" :disabled="!data.length||data.length<25" @click="clickNext">Next</v-btn>
    </div>
    <v-row class="mt-2">
      <template v-for="photo of data">
        <data-box :photo="photo" :key="photo.id" />
      </template>
      <v-card outlined height="400px" width="100%" class="text-center ma-4 " v-if="!data.length">
...
export default {
  ...
  data: () => ({
    ...
    roverTypes: [
      ...
      { name: 'MINITES', value: 10, desc:'Miniature Thermal Emission Spectrometer (Mini-TES)' },
    ],
